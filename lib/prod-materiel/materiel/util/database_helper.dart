...
class DatabaseHelper {
  ...
  final String tableNote = 'materiel';
  ...
  final String columndateajoutmateriel = 'dateajoutmateriel';
  ...
  initDb() async {
    ...
    String path = join(databasesPath, 'materiel.db');
...
//    await deleteDatabase(path); // just for testing
  ...
  void _onCreate(Database db, int newVersion) async {
    await db.execute(
        'CREATE TABLE $tableNote($columnId INTEGER PRIMARY KEY, $ColumnNom TEXT, $ColumnQte TEXT, $columnDetails TEXT, $columnPrixAchat TEXT, $columndateajoutmateriel TEXT, $columnstatut TEXT, $columnnomfournisseur TEXT)');
  }
...
//        'INSERT INTO $tableNote ($ColumnNom, $ColumnQte) VALUES (\'${note.title}\', \'${note.description}\')');
  ...
  Future<List> getAllNotes() async {
    var dbClient = await db;
    var result = await dbClient.query(tableNote, columns: [columnId, ColumnNom, ColumnQte, columnPrixAchat, columndateajoutmateriel, columnstatut, columnnomfournisseur],);
...
Future<List> getAllNotesMoisActuel() async {
    ...
    var result = await dbClient.rawQuery('SELECT * FROM $tableNote where  strftime("%m","$columndateajoutmateriel") = "0$moisActuel" ');
    return result.toList();
  }
  ...
  Future<List> getAllNotesMoisActueljMoins1() async {
    ...
    var dbClient = await db;
    // var result = await dbClient.query(tableNote, columns: [columnId, ColumnNom, ColumnQte,columnPrixVente, columnCoutTotal, columndateajoutmateriel] );
    var result = await dbClient.rawQuery('SELECT * FROM $tableNote where strftime("%m",$columndateajoutmateriel) = "0$moisActuelMoins1" ');
    return result.toList();
  }
  Future<List> getAllNotesMoisActuelMoins2() async {
    ...
    var dbClient = await db;
    // var result = await dbClient.query(tableNote, columns: [columnId, ColumnNom, ColumnQte,columnPrixVente, columnCoutTotal, columndateajoutmateriel] );
    var result = await dbClient.rawQuery('SELECT * FROM $tableNote where  strftime("%m",$columndateajoutmateriel) = "0$moisActuelMoins2" ');
    return result.toList();
  }  Future<List> getAllNotesMoisActuelMoins3() async {

    var dbClient = await db;
    // var result = await dbClient.query(tableNote, columns: [columnId, ColumnNom, ColumnQte,columnPrixVente, columnCoutTotal, columndateajoutmateriel] );
    var result = await dbClient.rawQuery('SELECT * FROM $tableNote where  strftime("%m",$columndateajoutmateriel) = "0$moisActuelMoins3" ');
    return result.toList();
  }
  Future<List> getAllNotesMoisActuelMoins4() async {
    ...
    var dbClient = await db;
    // var result = await dbClient.query(tableNote, columns: [columnId, ColumnNom, ColumnQte,columnPrixVente, columnCoutTotal, columndateajoutmateriel] );
    var result = await dbClient.rawQuery('SELECT * FROM $tableNote where  strftime("%m",$columndateajoutmateriel) = "0$moisActuelMoins4" ');
    return result.toList();
  }  Future<List> getAllNotesMoisActueljMoins5() async {

    var dbClient = await db;
    // var result = await dbClient.query(tableNote, columns: [columnId, ColumnNom, ColumnQte,columnPrixVente, columnCoutTotal, columndateajoutmateriel] );
    var result = await dbClient.rawQuery('SELECT * FROM $tableNote where  strftime("%m",$columndateajoutmateriel) = "$moisActuelMoins5" ');
    return result.toList();
  }
