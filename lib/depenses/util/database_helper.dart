...
class DatabaseHelperDepenses {
  static final DatabaseHelperDepenses _instance = new DatabaseHelperDepenses.internal();

  factory DatabaseHelperDepenses() => _instance;

  final String tableNote = 'depenses';
  ...
  DatabaseHelperDepenses.internal();
...
//    await deleteDatabase(path); // just for testing
  ...
  Future<int> saveNote(Depense note) async {
...
//    var result = await dbClient.rawQuery('SELECT * FROM $tableNote');
  ...
  Future<Depense> getNote(int id) async {
    var dbClient = await db;
...
//    var result = await dbClient.rawQuery('SELECT * FROM $tableNote WHERE $columnId = $id');
    ...
    if (result.length > 0) {
      return new Depense.fromMap(result.first);
    }
...
//    return await dbClient.rawDelete('DELETE FROM $tableNote WHERE $columnId = $id');
  ...
  Future<int> updateNote(Depense note) async {
    var dbClient = await db;
